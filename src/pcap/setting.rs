...
impl PacketCaptureSetting {
    pub fn from_interface_index(if_index: u32) -> Option<PacketCaptureSetting> {
        let iface = interface::get_interface_by_index(if_index)?;
        let options = PacketCaptureSetting {
            interface_index: iface.index,
            src_ips: HashSet::new(),
            dst_ips: HashSet::new(),
            src_ports: HashSet::new(),
    ...
    pub fn from_interface_name(if_name: String) -> PacketCaptureSetting {
        let iface = interface::get_interface_by_name(if_name).unwrap();
        let options: PacketCaptureSetting = PacketCaptureSetting {
            interface_index: iface.index,
            src_ips: HashSet::new(),
            dst_ips: HashSet::new(),
            src_ports: HashSet::new(),
    ...
    pub fn from_interface(iface: &Interface) -> PacketCaptureSetting {
        let options = PacketCaptureSetting {
            interface_index: iface.index,
            src_ips: HashSet::new(),
            dst_ips: HashSet::new(),

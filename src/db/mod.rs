...
pub fn get_default_ports() -> Vec<u16> {
    let default_ports: Vec<u16> = serde_json::from_str(config::DEFAULT_PORTS_JSON).expect("Invalid default-ports.json format");
    default_ports
}

pub fn get_wellknown_ports() -> Vec<u16> {
    let wellknown_ports: Vec<u16> = serde_json::from_str(config::WELLKNOWN_PORTS_JSON).expect("Invalid wellknown-ports.json format");
    wellknown_ports
}

pub fn get_http_ports() -> Vec<u16> {
    let http_ports: Vec<u16> = serde_json::from_str(config::HTTP_PORTS_JSON).expect("Invalid http-ports.json format");
    http_ports
}

pub fn get_https_ports() -> Vec<u16> {
    let https_ports: Vec<u16> = serde_json::from_str(config::HTTPS_PORTS_JSON).expect("Invalid https-ports.json format");
    https_ports
}

pub fn get_ttl_family_map() -> HashMap<u8, String> {
    let mut ttl_family_map: HashMap<u8, String> = HashMap::new();
    let ds_os_ttl: Vec<model::OsTtl> = serde_json::from_str(config::OS_TTL_JSON).expect("Invalid os-ttl.json format");
...
pub fn get_ttl_class_map() -> HashMap<u8, String> {
    ...
    let ds_os_ttl: Vec<model::OsClassTtl> = serde_json::from_str(config::OS_CLASS_TTL_JSON).expect("Invalid os-class-ttl.json format");
    for os_ttl in ds_os_ttl {
...
pub fn get_subdomain() -> Vec<String> {
    let subdomain: Vec<String> = serde_json::from_str(config::SUBDOMAIN_JSON).expect("Invalid subdomain.json format");
    subdomain
}

pub fn get_os_family_db() -> model::OsDb {
    let os_db : model::OsDb = serde_json::from_str(config::OS_FAMILY_FINGERPRINT_JSON).expect("Invalid os-family-fingerprint.json format");
    os_db
}
...
pub fn get_os_family_list() -> Vec<String> {
    let os_families: Vec<String> = serde_json::from_str(config::OS_FAMILY_JSON).expect("Invalid os-family.json format");
    os_families
}

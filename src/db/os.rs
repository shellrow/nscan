...
use anyhow::Result;
use crate::{config, os::{OsClass, OsClassTtl, OsDb}};

pub static OS_DB: OnceLock<OsDb> = OnceLock::new();

/// Initialize OS database
pub fn init_os_db() -> Result<()> {
    let os_db: OsDb = serde_json::from_str(config::db::OS_DB_JSON)
        .expect("Invalid nscan-os-db.json format");
    OS_DB
        .set(os_db)
...
pub fn get_ttl_class_map() -> HashMap<u8, String> {
    ...
    let ds_os_ttl: Vec<OsClassTtl> =
        serde_json::from_str(config::db::OS_CLASS_TTL_JSON).expect("Invalid os-class-ttl.json format");

...
use crate::{config::DEFAULT_LOCAL_UDP_PORT, neighbor::setting::AddressResolveSetting, ping::setting::PingSetting};
        ...
        match crate::interface::get_interface_by_index(ping_setting.if_index) {
            Some(interface) => {
                ...
                let src_ip = match ping_setting.dst_ip {
                    IpAddr::V4(_) => crate::interface::get_interface_ipv4(&interface).unwrap_or(IpAddr::V4(Ipv4Addr::LOCALHOST)),
                    IpAddr::V6(ipv6_addr) => {
                        if nex::net::ip::is_global_ipv6(&ipv6_addr) {
                            crate::interface::get_interface_global_ipv6(&interface).unwrap_or(IpAddr::V6(Ipv6Addr::LOCALHOST))
                        }else{
                            crate::interface::get_interface_local_ipv6(&interface).unwrap_or(IpAddr::V6(Ipv6Addr::LOCALHOST))
                        }
                    },
                };
                Self {
    ...
    pub fn from_address_resolve_settomg(resolve_setting: &AddressResolveSetting) -> Self {
        match crate::interface::get_interface_by_index(resolve_setting.if_index) {
            Some(interface) => {
                ...
                let src_ip = match resolve_setting.dst_ip {
                    IpAddr::V4(_) => crate::interface::get_interface_ipv4(&interface).unwrap_or(IpAddr::V4(Ipv4Addr::LOCALHOST)),
                    IpAddr::V6(ipv6_addr) => {
                        if nex::net::ip::is_global_ipv6(&ipv6_addr) {
                            crate::interface::get_interface_global_ipv6(&interface).unwrap_or(IpAddr::V6(Ipv6Addr::LOCALHOST))
